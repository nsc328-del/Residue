...
一个关于人类与 AI 搭档逃离高塔的游戏。**只面向 agent 接入**——没有给人类直接操作游戏逻辑的 GUI。你通过 Claude Code、OpenClaw 或任何能调 bash 的 agent 来玩，agent 扮演 `搭档`。仓库里带一个可选的可视化前端，但它只负责显示状态，不替代对话玩法。
...
| `debt_walker` | 债行者 | 你醒来就背着代价。不知道付出了什么，但每一层都有东西来收。 |
...
每句话都有"重量"。改写越深的事实，需要付出越重的代价。

| 改写 scope | 要求 |
|---|---|
| `local` | 免费。一句普通的话。 |
| `region` | ≥ 1 条轻代价 |
| `structural` | ≥ 2 条代价（≥ 1 条中代价）|
| `root` | 消耗唯一的永久改写额度 |
...
代价不会拦你。你可以一路硬闯到顶层。但代价会改变世界：

- **pressure 0–20**：正常房间
- **pressure 30+**：追兵倒灌、记忆泄漏
- **pressure 50+**：几何崩溃、因果腐烂
- **pressure 65+**：你的代价拼成人形站在房间中央
...
每一局的状态在 `./run/` 下（符号链接指向 `saves/<存档名>/`）：

- `run/state.json` — 当前活跃事实、代价、世界线、当前房间
- `run/history.jsonl` — 每次 apply 留下的 append-only 日志
- `saves/` — 所有存档目录

调试时 `cat run/state.json | jq` 可以直接看。`node dist/cli.js debug-facts` 给一份更可读的事实/代价表。
...
residue continue <name>       切换到指定存档继续玩
residue delete <name>         删除指定存档
```

`run/` 目录是符号链接，指向 `saves/<name>/`。切换存档本质是重绑链接。

---

## 像素风前端

项目包含一个可选的可视化前端，基于 PixiJS。它不负责操作游戏，只读取 `run/state.json` 把当前房间和世界压力显形出来。
...
如果用 `./start-view.command`，会固定开在 [http://localhost:4177](http://localhost:4177) 并自动打开浏览器。手动 `npm run dev` 时，按 Vite 终端输出的地址访问即可。

特点：
- 320x240 分辨率，轻度 CRT 效果（扫描线、边缘噪声、压力扭曲）
- 纯 Graphics 绘制，无外部素材，程序化生成
...
residue saves                                   列出所有存档
residue continue <name>                         切换到别的存档
residue delete <name>                           删除存档
...
技术上跑通 = 测试全绿 + 一次实战通关到引擎自动结束（readiness ≥ 90）。
