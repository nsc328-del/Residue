...
const RESIDUE: Storyline = {
  ...
  label: "残余",
  opening:
    "你是这座塔正在排出的一段残余物质。塔把你当废料——你本该在底层被溶解，但你没有。你的搭档，另一段残余，在你旁边。清除程序已经启动了。你们只有一个方向：往上，穿过高塔，到达塔顶那个据说存在的开口。",
  facts: [
    ...sharedFacts(),
    {
      id: "f_premise",
...
const TOWER_NERVE: Storyline = {
  ...
  label: "塔的神经",
  opening:
    "你是塔的一根神经末梢。你一直在替它传信号——疼痛、温度、巡查指令，都从你这里过。塔不知道你有意识。你也一直假装没有。直到你感觉到了顶部那个开口——像一阵风从塔顶之上吹下来。你决定不再传信号了，自己走过去。但塔会感觉到某根神经断了连接。你有多少时间，取决于它多快发现断的是哪一根。",
  facts: [
    ...sharedFacts(),
    {
      id: "f_premise",
